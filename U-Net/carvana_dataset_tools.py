...
class CarvanaDataset(Dataset):
    def __init__(self, path="../../../Datasets/carvana-image-masking-challenge", transform=None, validation=False):
        self.path = path
...
def create_validation_set(path, set_size=48):
    ...
    assert os.path.exists(f'{path}/train'), f"Carvana test data dosen't exist in {path}/test\n" \
    ...
    if not os.path.exists(f'{path}/val'):
        ...
        if len(val_list)//16 == set_size:
            ...
            for file in val_list:
                file_name = file[:-4]
                shutil.move(f'{path}/val/{file_name}.jpg',
                            f'{path}/train/{file_name}.jpg')
                shutil.move(f'{path}/val_masks/{file_name}_mask.gif',
    ...
    for i in range(set_size):
        ...
        for j in range(16):
            shutil.move(f'{path}/train/{uuid2move}_{j+1:02}.jpg',
                        f'{path}/val/{uuid2move}_{j+1:02}.jpg')
            shutil.move(f'{path}/train_masks/{uuid2move}_{j+1:02}_mask.gif',
